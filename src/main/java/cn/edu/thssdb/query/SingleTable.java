...
public class SingleTable extends QueryTable implements Iterator<Row> {
  ...
  public void prepareNext() {
    if (mLogicSelect == null) {
      ...
      if (theCondition == null) {
        ...
        } else if (leftComparer.getType() == ComparerType.COLUMN
            && rightComparer.getType() != ComparerType.COLUMN
        ...
        } else if (rightComparer.getType() == ComparerType.COLUMN
            && leftComparer.getType() != ComparerType.COLUMN
            && theCondition.getType() == ConditionType.EQ) {
          String primaryKeyName = mTable.getPrimaryName();
  ...
  /**
   * Prepare the next row based on the logic conditions. Rows are filtered based on the selection
   * logic and added to the queue.
   */
