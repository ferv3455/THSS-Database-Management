...
public class JointTable extends QueryTable implements Iterator<Row>{
    ...
    /**
     * Joins rows from multiple iterators to create a joint row.
     * If there are no more rows to be joined, it fetches new rows from the iterators.
     * The function handles the carry-over mechanism similar to addition, where it keeps resetting iterators
     * until it finds an iterator that has the next row to be joined.
     *
