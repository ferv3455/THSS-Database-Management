...
public class Table implements Iterable<Row> {
    ...
    /**
     * Validate if the input list of columns and entries are null or if their size matches the expected length.
     *
    ...
    private void validateInput(ArrayList<Column> columns, ArrayList<Entry> entries) {
        if (columns == null || entries == null) throw new LengthNotMatchException(this.columns.size(), 0);
     ...
     * <p>
     * This function is responsible for converting the Comparer's value to the
     * type defined by the column. It handles null values and validates the
     * input type to ensure compatibility with the column type. If a value is
     * provided for a non-nullable column, or if the value type does not match
     * the column type, exceptions are thrown.
     *
    ...
    /**
     * Validates if a new value is valid for a specific column.
     * It checks nullability and the maximum length (for STRING type only).
     *
     ...
     * @throws ValueLengthExceedException if the value is a string exceeding the maximum length defined by the column.
     */
    private void validateValue(Column column, Comparable newValue) {
        if (column.NotNull() && newValue == null) {
            throw new NullValueException(column.getName());
        }
        if (column.getType() == ColumnType.STRING && newValue != null && column.getMaxLength() >= 0 && newValue.toString().length() > column.getMaxLength()) {
     ...
     * @throws LengthNotMatchException    if the lengths of columns and values do not match
     *                                    or exceed the schema length.
    ...
    private ArrayList<Entry> prepareInsertion(String[] columns, String[] values) {
        ...
        int schemaLen = this.columns.size();
        if (columns.length > schemaLen || values.length > schemaLen || columns.length != values.length) {
        ...
        for (Column column : this.columns) {
            ...
            Comparable entryValue = (matches == 0 || index < 0 || index >= columns.length) ? null : ParseValue(column, values[index]);
    ...
    /**
     * This function is responsible for inserting values into the database, with an optional transaction parameter.
     * It aligns the incoming values with the schema of the table and inserts it into the appropriate columns.
     *
     * @param values        An array of String values to be inserted into the table.
     * @param isTransaction A boolean flag indicating whether the operation should be performed as a transaction.
     *                      Default is false, i.e., transaction is not used.
     * @throws LengthNotMatchException If the size of the values array is not consistent with the table schema.
    ...
    public void insert(String[] values, boolean isTransaction) {
        ...
        ArrayList<Entry> orderedEntries = new ArrayList<>();
        for (int i = 0; i < this.columns.size(); i++) {
            Comparable the_entry_value = i < values.length ? ParseValue(this.columns.get(i), values[i]) : null;
    ...
    //This function is a simplified version of the above insert function where the operation is always non-transactional
    ...
    /**
     * Validates the primary entry parameter.
     * Throws an exception if the primary entry is null.
     *
     ...
     * This operation is non-transactional.
     *
     ...
     */
    public void update(Entry primaryEntry, ArrayList<Column> updateColumns, ArrayList<Entry> updateEntries, boolean isTransaction) {
    ...
    /**
     * Updates the rows in the table based on the specified column name, value, logic condition, and transaction flag.
     *
     ...
     * @param isTransaction Indicates whether the update operation should be executed as a transaction.
